...
import { getAdminUserDocuments } from '@/lib/admin/users';
...
export const metadata = {
  title: 'User Documents · Bilyo Admin',
  description: 'Inspect all invoices and quotations issued by this account in read-only mode.',
};

interface AdminUserDocumentsPageProps {
  params: Promise<{ id: string }>;
}

export default async function AdminUserDocumentsPage({
  params,
}: AdminUserDocumentsPageProps) {
  await requireAdmin();
  const { id } = await params;

  const data = await getAdminUserDocuments(id);
  if (!data) {
    notFound();
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="text-sm text-slate-500 font-medium bg-slate-100 px-3 py-1.5 rounded-md self-start sm:self-auto">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-xs text-slate-600 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span>
            ...
            or delete documents belonging to this account.
          </span>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
        {documents.length === 0 ? (
          <div className="py-12 text-center text-slate-500 text-sm">
            ...
            No invoices or quotations created by this user yet.
          </div>
        ) : (
            ...
            <table className="min-w-full divide-y divide-slate-200 text-xs">
              <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider font-semibold">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left">Due / Valid</th>
              ...
              <tbody className="divide-y divide-slate-200 bg-white">
                {documents.map((doc) => {
                  ...
                  return (
                    <tr key={`${doc.kind}-${doc.id}`} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-right">
                        <Link
                          href={`/admin/documents/${doc.kind}/${doc.id}`}
