...
export function ClientForm({ initialCustomer }: ClientFormProps) {
  ...
  const handleToggleArchive = () => {
    ...
    const confirmMessage = targetState
      ? 'Archive this client? They will be hidden from new quotation pickers.'
      : 'Restore this client?';
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <form
        ...
        className="bg-white border border-[var(--color-line)] rounded-xl shadow-xs p-6 sm:p-8 space-y-5"
      >
        <div className="border-b border-[var(--color-line)] pb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-neutral-500 mt-0.5">
              Client details can be selected when creating quotations.
            </p>
          </div>
        ...
        <div className="pt-4 border-t border-[var(--color-line)] flex items-center justify-between">
          <div>
            {isEditing && (
              ...
              >
                {initialCustomer?.archived ? 'Restore client' : 'Archive client'}
              </button>
            )}
          </div>
