...
export function CustomerList({ initialCustomers }: CustomerListProps) {
  ...
  const handleToggleArchive = (customer: SerializedCustomer) => {
    const targetState = !customer.archived;
    const msg = targetState
      ? `Archive "${customer.name}"? They will be hidden from document pickers.`
      : `Restore "${customer.name}"?`;

    if (!confirm(msg)) return;

    startTransition(async () => {
      const res = await archiveCustomer(customer.id, targetState);
      if (res.ok) {
        setCustomers((prev) =>
          prev
    ...
    <div className="space-y-4">
      ...
      {error && (
        <div className="p-3 text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg flex items-center justify-between">
          <span>{error}</span>
          <button type="button" onClick={() => setError(null)} className="text-red-500 hover:text-red-700 text-xs font-medium">Dismiss</button>
        </div>
      )}
        ...
        <>
          ...
          <div className="md:hidden space-y-3">
            {customers.map((c) => (
              <div
                key={c.id}
                className={`bg-white border border-[var(--color-line)] rounded-xl p-4 shadow-sm space-y-2.5 ${
